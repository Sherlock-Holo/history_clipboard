...
pub struct CustomRadio<T> {
    child: WidgetPod<T, Box<dyn Widget<T>>>,
    variant: T,
    style: Box<dyn StyleSheet<T>>,
}

impl<T> CustomRadio<T> {
    pub fn new<W: Widget<T> + 'static>(child: W, variant: T) -> Self {
        Self {
...
macro_rules! style_from_ctx {
    ($ctx:expr, $style:expr, $variant:expr, $data:expr) => {
        if $ctx.is_disabled() {
            $style.disabled()
        } else {
            if $ctx.is_active() || $variant.same($data) {
                $style.chosen()
            } else if $ctx.is_hot() {
                $style.hovered()
            } else {
...
impl<T: Data> Widget<T> for CustomRadio<T> {
    ...
    fn update(&mut self, ctx: &mut UpdateCtx, old_data: &T, data: &T, env: &Env) {
        ...
        let mut style = style_from_ctx!(ctx, self.style, self.variant, data);
